...
perl run.pl JOB=1:$num_jobs $dir/log/get_phone_alignments.JOB.log \
...
if ! perl run.pl $dir/log/analyze_alignments.log \
  gunzip -c "$dir/phone_stats.*.gz" \| \
  python analyze_phone_length_stats.py $lang; then
