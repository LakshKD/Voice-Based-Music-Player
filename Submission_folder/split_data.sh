...
./split_scp.pl $utt2spk_opt $data/utt2spk $utt2spks || exit 1

for n in `seq $numsplit`; do
  dsn=$data/split${numsplit}${utt}/$n
  perl utt2spk_to_spk2utt.pl $dsn/utt2spk > $dsn/spk2utt || exit 1;
done
...
for f in feats.scp text vad.scp utt2lang $maybe_wav_scp; do
  if [ -f $data/$f ]; then
    perl filter_scps.pl JOB=1:$numsplit \
...
for f in spk2gender spk2warp cmvn.scp; do
  if [ -f $data/$f ]; then
    perl filter_scps.pl $warning_opt JOB=1:$numsplit \
