...
perl run.pl JOB=1:$num_jobs $dir/log/lattice_best_path.JOB.log \
...
perl run.pl JOB=1:$num_jobs $dir/log/get_lattice_stats.JOB.log \
...
perl run.pl $dir/log/analyze_alignments.log \
  gunzip -c "$dir/phone_stats.*.gz" \| \
  ./analyze_phone_length_stats.py $lang || exit 1
...
perl run.pl $dir/log/analyze_lattice_depth_stats.log \
  gunzip -c "$dir/depth_stats_tmp.*.gz" \| \
  python analyze_lattice_depth_stats.py $lang || exit 1
